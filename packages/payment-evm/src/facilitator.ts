...
import {
  ...
  EIP712_TYPES,
  X402_EXACT_PAYLOAD_SCHEMA,
} from "./constants";

const validateX402ExactPayload = type(X402_EXACT_PAYLOAD_SCHEMA);
...
function parseSignature(signature: string): { v: number; r: Hex; s: Hex } {
  // EIP-712 signatures are 65 bytes: r (32 bytes) + s (32 bytes) + v (1 byte)
...
export function createFacilitatorHandler(
  ...
  const handleSettle = async (
    ...
    const payloadResult = validateX402ExactPayload(payment.payload);
    if (isValidationError(payloadResult)) {
    ...
    try {
      onChainUsed = await publicClient.readContract({
        ...
        args: [authorization.from as Hex, authorization.nonce as Hex],
      });
    } catch (error) {
    ...
    // Execute the transaction. Must be a local account to sign locally. Using
    // just an address would trigger eth_signTransaction RPC call.
    ...
    try {
      ...
      // Sign the transaction locally (wallet client has the private key)
