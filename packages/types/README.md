...
| `schemes`         | `string[] or undefined`                                                                                                                                                                                                                                                                                                               | x402 schemes this handler can settle. Required for in-process resolution; handlers without it are skipped by the middleware's dispatch path. Distinct from {@link getSupported}, which exists for the facilitator `/supported` HTTP route and may fan out to remote backends with per-kind timeouts. |
...
| Type                  | Type                                        |
| --------------------- | ------------------------------------------- |
| `HandlerCapabilities` | `{ networks: string[]; assets: string[]; }` |
