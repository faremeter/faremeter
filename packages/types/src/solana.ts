...
/**
 * Validator for Solana CAIP-2 network identifiers.
 *
 * Format: solana:<genesis-hash> where genesis-hash is base58-encoded.
 */
export const SolanaCAIP2Network = type(/^solana:[1-9A-HJ-NP-Za-km-z]+$/);
export type SolanaCAIP2Network = typeof SolanaCAIP2Network.infer;

/**
 * Type guard that checks if a value is a valid Solana CAIP-2 network identifier.
 *
 * @param maybe - The value to check
...
export function isSolanaCAIP2Network(
  ...
  return !isValidationError(SolanaCAIP2Network(maybe));
}
